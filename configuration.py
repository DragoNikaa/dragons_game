from dataclasses import dataclass

import pygame

pygame.init()
screen = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)
clock = pygame.time.Clock()


@dataclass
class General:
    SCREEN_WIDTH = screen.get_width()
    SCREEN_HEIGHT = screen.get_height()
    NAME = "Dragons"
    # ICON = pygame.image.load('').convert_alpha()


pygame.display.set_caption(General.NAME)
# pygame.display.set_icon(General.ICON)


@dataclass
class StartScreen:
    BACKGROUNDS_DIR_PATH = "graphics/start_screen_backgrounds"

    NAME_FONT_PATH = "fonts/pr_viking.ttf"
    NAME_SIZE = int(General.SCREEN_HEIGHT / 3.5)
    NAME_COLOR = "white"
    NAME_DEST = (General.SCREEN_WIDTH / 2, General.SCREEN_HEIGHT / 3.5)
    NAME_ANTIALIAS = True

    NAME_BORDER_COLOR = "black"
    NAME_BORDER_THICKNESS = int(General.SCREEN_HEIGHT / 60)

    TEXT_BUTTON_TEXT_FONT_PATH = "fonts/rurik.ttf"
    TEXT_BUTTON_TEXT_SIZE = int(General.SCREEN_HEIGHT / 9)
    TEXT_BUTTON_TEXT_COLOR = "white"
    TEXT_BUTTON_TEXT_ANTIALIAS = True

    TEXT_BUTTON_TEXT_BORDER_COLOR = 'black'
    TEXT_BUTTON_TEXT_BORDER_THICKNESS = int(General.SCREEN_HEIGHT / 160)

    TEXT_BUTTON_WIDTH = General.SCREEN_WIDTH / 4
    TEXT_BUTTON_HEIGHT = General.SCREEN_HEIGHT / 4
    TEXT_BUTTON_IMAGE_PATH = "graphics/button_backgrounds/text_button.png"
    TEXT_BUTTON_Y_OFFSET = TEXT_BUTTON_HEIGHT / 6

    START_BUTTON_TEXT = "START"
    START_BUTTON_DEST = (General.SCREEN_WIDTH / 2, General.SCREEN_HEIGHT / 1.5)


@dataclass
class LevelSelection:
    BACKGROUNDS_DIR_PATH = 'graphics/level_selection_backgrounds'

    LEVEL_BUTTON_WIDTH = LEVEL_BUTTON_HEIGHT = General.SCREEN_HEIGHT / 8
    LEVEL_BUTTON_IMAGE = 'graphics/button_backgrounds/level_button.png'
    LEVEL_BUTTONS_NUMBER = 4
    LEVEL_BUTTONS_DEST = {1: ((General.SCREEN_WIDTH / 5.80, General.SCREEN_HEIGHT / 1.95),
                              (General.SCREEN_WIDTH / 2.70, General.SCREEN_HEIGHT / 3.70),
                              (General.SCREEN_WIDTH / 1.63, General.SCREEN_HEIGHT / 1.80),
                              (General.SCREEN_WIDTH / 1.20, General.SCREEN_HEIGHT / 2.70))}
